...
public class IPSeeker {
	private final Logger logger = LoggerFactory.getLogger(IPSeeker.class);
	// 纯真IP数据库名
	 ...
	 */
	public List<IPEntry> getIPEntryDebug(String s) {
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (ipLoc.getArea().indexOf(s) != -1 || ipLoc.getOperator().indexOf(s) != -1) {
					IPEntry entry = new IPEntry();
					entry.setArea(ipLoc.getArea());
					entry.setOperator(ipLoc.getOperator());
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.setBeginIP(this.getIpStringFromBytes(b4));
					// 得到结束IP
					readIP(temp, b4);
					entry.setEndIP(this.getIpStringFromBytes(b4));
					// 添加该记录
					ret.add(entry);
				}
			}
	 ...
	 * @return 包含IPEntry类型的List
	 */
	public List<IPEntry> getIPEntry(String s) {
		List<IPEntry> ret = new ArrayList<IPEntry>();
		try {
			// 映射IP信息文件到内存中
			if (mbb == null) {
	...
	public IPLocation getIPLocation(String ip) {
		if (!NetUtil.isIPv4Address(ip))
			return new IPLocation();

		IPLocation location = new IPLocation();
		location.setArea(this.getArea(ip));
		location.setOperator(this.getOperator(ip));
		return location;
	}
	...
	public String getArea(String ip) {
		if (!NetUtil.isIPv4Address(ip))
			return "";
		return getArea(this.getIpByteArrayFromString(ip));
	}
	 ...
	 * @return 运营商
	 */
	public String getOperator(String ip) {
		if (!NetUtil.isIPv4Address(ip))
			return "";
		return getOperator(this.getIpByteArrayFromString(ip));
	}
	 ...
	 * @return 运营商
	 */
	public String getOperator(byte[] ip) {
		// 检查ip地址文件是否正常
		if (ipFile == null)
			return Message.bad_ip_file;
		// 保存ip，转换ip字节数组为字符串形式
	...
	private IPLocation getIPLocation(byte[] ip) {
		...
		if (info == null) {
			...
			info.setOperator(Message.unknown_operator);
		}
		return info;
	}
		...
		try {
			...
			if (b == REDIRECT_MODE_1) {
				...
				if (b == REDIRECT_MODE_2) {
					loc.setArea(readString(readLong3()));
					ipFile.seek(countryOffset + 4);
				} else
					loc.setArea(readString(countryOffset));
				// 读取运营商标志
				loc.setOperator(readOperator(ipFile.getFilePointer()));
			} else if (b == REDIRECT_MODE_2) {
				loc.setArea(readString(readLong3()));
				loc.setOperator(readOperator(offset + 8));
			} else {
				loc.setArea(readString(ipFile.getFilePointer() - 1));
				loc.setOperator(readOperator(ipFile.getFilePointer()));
			}
			return loc;
		} catch (IOException e) {
	...
	private IPLocation getIPLocation(int offset) {
		...
		if (b == REDIRECT_MODE_1) {
			...
			b = mbb.get();
			if (b == REDIRECT_MODE_2) {
				loc.setArea(readString(readInt3()));
				mbb.position(countryOffset + 4);
			} else
				loc.setArea(readString(countryOffset));
			// 读取运营商标志
			loc.setOperator(readOperator(mbb.position()));
		} else if (b == REDIRECT_MODE_2) {
			loc.setArea(readString(readInt3()));
			loc.setOperator(readOperator(offset + 8));
		} else {
			loc.setArea(readString(mbb.position() - 1));
			loc.setOperator(readOperator(mbb.position()));
		}
		return loc;
	}
	 ...
	 * @throws IOException
	 */
	private String readOperator(long offset) throws IOException {
		ipFile.seek(offset);
		byte b = ipFile.readByte();
	 ...
	 * @return 地区名字符串
	 */
	private String readOperator(int offset) {
		mbb.position(offset);
		byte b = mbb.get();
		if (b == REDIRECT_MODE_1 || b == REDIRECT_MODE_2) {
